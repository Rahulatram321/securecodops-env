"""
SecureCodeOps Inference Script
Compliant with OpenEnv Hackathon submission requirements.
"""
...
# ── Required env vars — EXACT names the validator checks ──────────────────
...
# OpenAI client — uses HF_TOKEN as the API key (organizer proxy injects correct key)
client = OpenAI(base_url=API_BASE_URL, api_key=HF_TOKEN)


# ── Logging helpers — flush=True is MANDATORY so evaluator sees output ────
...
def log_end(success: bool, steps: int, rewards: List[float]) -> None:
    ...
    # NOTE: [END] format has NO score= field — only success, steps, rewards
    print(f"[END] success={success_str} steps={steps} rewards={rewards_str}", flush=True)


# ── LLM ping — ensures the proxy records API usage for evaluation ─────────
def llm_proxy_ping() -> None:
    try:
        ...
        import sys
        print(f"[WARN] LLM ping failed: {e}", file=sys.stderr, flush=True)
